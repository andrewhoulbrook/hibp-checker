...
# Check a password hash in breached datasets via the HIBP API
function checkPassword() {
    read -sp "Enter password: " PASSWORD_STR   # don't print to terminal

    # HIBP API uses k-anonymity method to query password hashes
    ...
    # Pass first five digits of password hash (SHA-1) to API, grep results for an exact match on full password hash
    ...
    # Print matched result from HIBP response. If match found, print number of times password is found in HIBP datasets  
    printf "\nWARNING: password appears in HIBP datasets: %d times.\n" "${RESULT#*:}" 2>/dev/null
}

# Check an email account in breached datasets via the HIBP API
function checkAccount() {
    ...
    # Pass user-agent header otherwise the API returns a HTTP 403 respones
